...
#Function used to list all available disks, then prompt to format the disk or not
...
#Convert ISO to IMG.DMG format function
convert_iso (){ hdiutil convert -format UDRW -o $1 $2 }

#Function used to burn the IMG to disk wheather it has been converted previously or not.
burn_to_disk (){
    echo "Has the ISO been previously converted to IMG? (y/n)"
    read answer
    ...
    case "$answer" in 
        ...
        ;;
        *) 
        echo "You have entered an invalid option. The program will now terminate."
        exit 0
